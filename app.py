...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v694")
    if st.button("进入系统"):
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("📝 修正持仓"):
            with st.form("fix_v6", clear_on_submit=True):
...
def render_ledger(target):
    ...
    with ci:
        ...
        with st.form(key=f"form_led_v694_{ty}", clear_on_submit=True):
            ...
            if st.form_submit_button("确认存入"):
                ...
                if clean_v:
                    try:
                        tm = round(float(clean_v), 4); pn, pc = ta.split('|')
    ...
    with cl:
        ...
        if logs:
            ...
            d_cols = ["时间", "分类", "账户", "类型", "金额", "币种", "等值USD", "备注"]
            st.dataframe(df_l[[c for c in d_cols if c in df_l.columns]], use_container_width=True, hide_index=True)
...
if st.session_state.auth_role == "admin":
    ...
    with t3:
        if logs:
            ...
            curr_m = st.selectbox("选择月份", sorted(df['Month'].unique(), reverse=True)); df_m = df[df['Month'] == curr_m]
