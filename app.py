...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v693")
    if st.button("进入系统"):
    ...
    with open(f, 'r', encoding='utf-8') as fs:
        try:
            ...
            if f == 'transactions.json':
                ...
                for e in data:
                    t_str = str(e.get('时间', ''))
                    if len(t_str) > 16: e['时间'] = t_str[:16] # 强制去秒
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("➕ 新增资产"):
            with st.form("add_final", clear_on_submit=True):
                ...
                if st.form_submit_button("确认添加"):
                    try:
                        n_clean = na_raw.replace(',', '').strip()
                        if n_clean and np:
                            na = round(float(n_clean), 4)
...
def render_ledger(target):
    ...
    with ci:
        ...
        # 强制独立 Form ID，确保状态不冲突
        with st.form(key=f"ledger_form_{ty}_v693", clear_on_submit=True):
            ...
            if st.form_submit_button("确认存入"):
                ...
                if not clean_val:
                    ...
                    try:
                        tm = round(float(clean_val), 4); pn, pc = ta.split('|')
                        ...
                        st.error(f"❌ '{tm_raw}' 不是有效数字，请重新输入")

    with cl:
        st.subheader("📜 历史流水 (去秒展示)")
        if logs:
            df_l = pd.DataFrame(logs).head(50)
            disp_cols = ["时间", "分类", "账户", "类型", "金额", "币种", "等值USD", "备注"]
            st.dataframe(df_l[disp_cols], use_container_width=True, hide_index=True)
