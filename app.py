...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v710")
    if st.button("进入系统"):
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("📝 修正持仓"):
            with st.form("fix_v710", clear_on_submit=True):
        ...
        with st.expander("➕ 新增资产"):
            with st.form("add_v710", clear_on_submit=True):
                na_raw = st.text_input("初始金额", placeholder="输入数字"); np = st.text_input("平台名称")
...
def render_ledger(target):
    ...
    with ci:
        ...
        # 引入物理占位符抹除假报错
        msg_box = st.empty()
        with st.form(key=f"form_v710_{ty}", clear_on_submit=True):
            tc = st.selectbox("分类", ["🚬 烟酒", "🍚 外餐", "🎰 德州", "🏠 房租", "🛒 购物", "🛠️ 其他"] if ty=="支出" else ["💰 工资", "📈 投资", "🃏 德州盈利", "🎁 报销", "🔄 收入"])
            ta = st.selectbox("账户", opt_list); tm_raw = st.text_input("金额", placeholder="数字...", key=f"v_inp_{ty}"); tn = st.text_input("备注")
            if st.form_submit_button("确认存入"):
                ...
                if clean_v:
                    try:
                        ...
                        # 核心修正：入库成功后瞬间物理刷新，绝不给空值报错机会
                        st.rerun()
                    except: msg_box.error("⚠️ 金额框必须填纯数字")
                else: msg_box.warning("⚠️ 金额不能为空")
    with cl:
        st.subheader("📜 历史流水")
        if logs:
