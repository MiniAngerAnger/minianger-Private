...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v715")
    if st.button("进入系统"):
        if pwd == "13571357":
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            r = {
                ...
                "USD_TO_IDR": f.get('IDR', 15800)/f.get('USD', 0.138),
                "USD_TO_CNY": 1/f.get('USD', 0.138)
            }
    except:
        pass
    return r
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("📝 修正持仓"):
            with st.form("fix_v715", clear_on_submit=True):
        ...
        with st.expander("➕ 新增资产"):
            with st.form("add_v715", clear_on_submit=True):
                na_raw = st.text_input("金额", placeholder="输入数字")
...
if st.session_state.auth_role == "admin":
    ...
    with t3:
        if logs:
            ...
            exp_m = df_m[df_m['类型'] == '支出']['等值USD'].sum()
            inc_m = df_m[df_m['类型'] == '收入']['等值USD'].sum()
            m1, m2, m3 = st.columns(3)
            m1.metric("🔴 支出", f"${exp_m:,.2f}")
            m2.metric("🟢 收入", f"${inc_m:,.2f}")
            m3.metric("⚖️ 盈亏", f"${inc_m - exp_m:,.2f}", delta=float(inc_m - exp_m))
            st.divider()
            cl, cr = st.columns(2)
            with cl:
                st.write("### 支出构成")
