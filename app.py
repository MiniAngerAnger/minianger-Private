...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v702")
    if st.button("进入系统"):
        if pwd == "13571357":
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("📝 修正持仓"):
            with st.form("fix_v702", clear_on_submit=True):
...
def render_ledger(target):
    ...
    with ci:
        ...
        msg_area = st.empty()
        with st.form(key=f"led_v702_{ty}", clear_on_submit=True):
            tc = st.selectbox("分类", ["🚬 烟酒", "🍚 外餐", "🎰 德州", "🏠 房租", "🛒 购物", "🛠️ 其他"] if ty=="支出" else ["💰 工资", "📈 投资", "🃏 德州盈利", "🎁 报销", "🔄 收入"])
            ta = st.selectbox("账户", opt_list)
            tm_raw = st.text_input("金额", placeholder="数字...", key=f"v_{ty}")
            tn = st.text_input("备注")
            if st.form_submit_button("确认存入"):
                ...
                if clean_v:
                    try:
                        ...
                        save_db('assets.json', assets)
                        # 数据一旦成功写入，强制中断当前逻辑执行，直接刷新页面
                        st.rerun()
                    except:
                        msg_area.error("⚠️ 请输入纯数字")
                else:
