...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v689")
    if st.button("进入系统"):
...
with st.sidebar:
    ...
    if st.session_state.auth_role == "admin":
        ...
        with st.expander("📝 修正持仓"):
            with st.form("fix_assets_v2", clear_on_submit=True):
        ...
        with st.expander("➕ 新增资产"):
            with st.form("add_asset_v2", clear_on_submit=True):
...
def render_ledger(target):
    ...
    with ci:
        ...
        if ty == "支出":
            with st.form("form_exp_final", clear_on_submit=True):
                tc = st.selectbox("分类", ["🚬 烟酒", "🍚 外餐", "🎰 德州", "🏠 房租", "🛒 购物", "🛠️ 其他"])
                ta = st.selectbox("账户", opt_list); tm_raw = st.text_input("金额", placeholder="数字..."); tn = st.text_input("备注")
                if st.form_submit_button("确认支出"):
                    try:
                        tm = round(float(tm_raw), 4); pn, pc = ta.split('|')
                        uv = round(tm * rates.get(pc, 1.0), 4); cv = round(uv * rates.get('USD_TO_CNY', 7.23), 2)
                        logs.insert(0, {"时间": get_time(), "分类": tc, "账户": pn, "类型": "支出", "金额": tm, "币种": pc, "等值USD": uv, "等值CNY": cv, "备注": tn})
                        save_db('transactions.json', logs)
                        for ck in assets:
                            for i in assets[ck]:
                                if i['platform'] == pn and i['currency'] == pc: i['amount'] = round(i['amount'] - tm, 4)
                        save_db('assets.json', assets); st.rerun()
                    except: st.error("数字错误")
        else:
            with st.form("form_inc_final", clear_on_submit=True):
                tc = st.selectbox("分类", ["💰 工资", "📈 投资", "🃏 德州盈利", "🎁 报销", "🔄 收入"])
                ta = st.selectbox("账户", opt_list); tm_raw = st.text_input("金额", placeholder="数字..."); tn = st.text_input("备注")
                if st.form_submit_button("确认收入"):
                    try:
                        tm = round(float(tm_raw), 4); pn, pc = ta.split('|')
                        uv = round(tm * rates.get(pc, 1.0), 4); cv = round(uv * rates.get('USD_TO_CNY', 7.23), 2)
                        logs.insert(0, {"时间": get_time(), "分类": tc, "账户": pn, "类型": "收入", "金额": tm, "币种": pc, "等值USD": uv, "等值CNY": cv, "备注": tn})
                        save_db('transactions.json', logs)
                        for ck in assets:
                            for i in assets[ck]:
                                if i['platform'] == pn and i['currency'] == pc: i['amount'] = round(i['amount'] + tm, 4)
    ...
    with cl:
        ...
        if logs:
            ...
            st.dataframe(df_l[["时间", "分类", "账户", "类型", "金额", "币种", "等值USD", "备注"]], use_container_width=True, hide_index=True)
...
if st.session_state.auth_role == "admin":
    ...
    with t3:
        if logs:
            ...
            curr_m = st.selectbox("月份筛选", sorted(df['Month'].unique(), reverse=True)); df_m = df[df['Month'] == curr_m]
    ...
    st.title("📝 记账助手 (协作版)"); render_ledger(st)
