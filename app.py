...
def check_auth():
    ...
    pwd = st.text_input("请输入访问口令:", type="password", key="auth_v722")
    if st.button("进入系统"):
        if pwd == "13571357":
                ...
                if st.form_submit_button("确认添加"):
                    try:
                        ...
                        if np:
                            ...
                            save_db('assets.json', assets)
                            st.rerun()
                    except: st.error("金额错误")
        with st.expander("🗑️ 移除资产"):
            with st.form("del_v722"):
...
def render_ledger(target):
    ...
    with ci:
        ...
        with st.form(key=f"form_722_{ty}", clear_on_submit=False):
            tc = st.selectbox("分类", ["🚬 烟酒", "🍚 外餐", "🎰 德州", "🏠 房租", "🛒 购物", "🛠️ 其他"] if ty=="支出" else ["💰 工资", "📈 投资", "🃏 德州盈利", "🎁 报销", "🔄 收入"])
            ta = st.selectbox("账户", opt_list)
            tm_raw = st.text_input("金额", placeholder="输入数字...", key=f"inp_chrome_{ty}")
            tn = st.text_input("备注")
